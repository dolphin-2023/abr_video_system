...
DEFAULT_SOURCE_ROOT = PROJECT_ROOT / "origin_traces" / "new"
...
def build_puffer_recent(
    ...
    for sent_csv, stream_key, values, stats in candidates:
        ...
        write_trace(
            target,
            values,
            "Recent Puffer delivery_rate bytes/s converted to Kbps; external test only",
        )
...
def build_weak_mobile(
    ...
    for source_name, source_item, values, stats in candidates:
        ...
        write_trace(target, values, "Weak mobile Kbps trace; external test only")
        seen.add(stats["digest"])
        selected.append({
            "source": source_name,
...
def parse_args():
    parser = argparse.ArgumentParser(description="Import downloaded external ABR test datasets.")
