...
def _first_bool(mapping, keys, default=None):
    for key in keys:
    ...
    def describe(self):
        if self.uses_normalized_reward:
            return (
                ...
                f"return_scale={self.return_scale:g}, "
                f"clip_reward={self.clip_reward}, source={self.source}"
            )
...
def load_return_to_go_processor(
    ...
    resolved_clip_reward = (
        bool(clip_reward)
        if clip_reward is not None
        else _first_bool(stats, ("clip_normalized_reward", "clip_reward"), True)
    )
    ...
    return ReturnToGoProcessor(
        ...
        clip_reward=bool(resolved_clip_reward),
        source=source,
    )
