...
_LIGHTWEIGHT_POLICIES = {"bba", "bola", "mpc", "random"}
...
class BbaPolicy:
    ...
    def __init__(self, reservoir=5.0, cushion=15.0):
    ...
    def act(self, state, env):
        ...
        min_br = float(min(bitrates))
        max_br = float(max(bitrates))
        ...
        if buffer_level <= self.reservoir:
            ...
            target_kbps = min_br + ratio * (max_br - min_br)

        action = int(np.argmin(np.abs(np.asarray(bitrates, dtype=np.float32) - target_kbps)))
        return max(0, min(env.num_bitrates - 1, action))
...
def run_evaluation(
    ...
    policies = list(policies or ["bba", "bola", "mpc", "random"])
    if "all" in {item.lower() for item in policies}:
        policies = ["bba", "bola", "mpc", "random", "pensieve", "netllm-sft", "netllm-offline-rl", "netllm-rl"]
...
def parse_args():
    ...
    parser.add_argument(
        "--policies",
        nargs="+",
        default=["bba", "bola", "mpc", "random"],
        help="Available: bba bola mpc random pensieve netllm-sft netllm-offline-rl netllm-rl all",
    )
    ...
    parser.add_argument("--workers", type=int, default=1,
                        help="Parallel workers for bba/bola/mpc/random (heavy models stay serial).")
