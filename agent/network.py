...
class NetLLMABR(nn.Module):
    ...
    def __init__(
        ...
        conv_out_dim = state_embed_dim * (self.past_k - conv_size + 1)
    ...
    def forward(self, states, actions, returns, timesteps, attention_mask=None):
        ...
        stacked_inputs = stacked_inputs[:, -self.context_token_limit(include_current_action=True) :, :]
    ...
    def predict_logits_batch(self, states, target_returns, timesteps, histories=None):
        ...
        stacked = stacked[:, -self.context_token_limit(include_current_action=False):, :]
    ...
    def new_history_tuple(self):
        return (
            deque([torch.zeros(1, 0, self.plm_embed_size)], maxlen=self.context_len),
            deque([torch.zeros(1, 0, self.plm_embed_size)], maxlen=self.context_len),
            deque([torch.zeros(1, 0, self.plm_embed_size)], maxlen=self.context_len),
        )

    def context_token_limit(self, include_current_action):
        current_tokens = 8 if include_current_action else 7
        return max(self.context_len * 8, current_tokens)
